...
def _load_api_key():
    ...
    if os.path.exists(key_file):
        ...
        for i, line in enumerate(lines):
            if line.lower() == "chatgpt:" and i + 1 < len(lines):
                return lines[i + 1]

        # Fallback: first OpenAI-style key found.
        for line in lines:
...
chatGPT_client = OpenAI(api_key=API_KEY) if API_KEY else None

MAX_REQUEST_DELAY_SEC = 0.12
DEFAULT_MODEL_NAME = "gpt-5.4"
MAX_RESPONSE_RETRIES = 3
ENABLE_TEMPERATURE_SWEEP = False
TEMPERATURE_SWEEP_ITERATIONS = 2
CHATGPT_TEMPERATURE_MIN = 0.0
CHATGPT_TEMPERATURE_MAX = 2.0
CHATGPT_TEMPERATURE_STEP = 0.1
...
def prompt_chatgpt(requirement_text, model_name, temperature=0.0):
...
def run_evaluation_once(
    ...
    for item in data:
        ...
        llm_output = prompt_chatgpt(item['description'], model_name, temperature=temperature)
...
def run_evaluation(model_name=DEFAULT_MODEL_NAME, iterations=2):
    ...
    aggregated_file = os.path.join(output_dir, 'chatgpt_results_aggregated.json')
...
def run_temperature_sweep(model_name=DEFAULT_MODEL_NAME):
    ...
    for temperature in temperature_values:
        ...
        print(f"\n{'='*50}")
        print(f"Temperature sweep at {temperature:.1f}")
        print(f"{'='*50}")

        for iteration in range(1, TEMPERATURE_SWEEP_ITERATIONS + 1):
            print(f"Running iteration {iteration}/{TEMPERATURE_SWEEP_ITERATIONS} at temperature {temperature:.1f}")
            iteration_results = run_evaluation_once(
                model_name,
                data,
                model_metadata,
        ...
        temp_file = os.path.join(
            output_dir,
            f"chatgpt_results_temperature_{temperature:.1f}_aggregated.json",
        )
        with open(temp_file, 'w') as f:
            json.dump(temp_aggregate, f, indent=4)
        print(f"Saved temperature aggregate to {temp_file}")

    aggregates_file = os.path.join(output_dir, 'chatgpt_results_temperature_aggregates.json')
    ...
    final_file = os.path.join(output_dir, 'chatgpt_results_temperature_sweep_aggregated.json')
